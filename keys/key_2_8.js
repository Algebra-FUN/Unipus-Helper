window.key_2_8 = {

    'key-8-2-7': {

        type: 'blank',

        content: `1.	arresting  2. omits  3. optional   4. transplanted  5. hence   
6. twisted  7. stung   8. minute  9. imitate   10. evil 

    `

    },

    'key-8-2-9': {

        type: 'blank',

        content: `interference/thriller   /scarce    /specific
            / consumption / deception / invasion / procession 
`
    },

    'key-8-2-10': {

        type: 'blank',

        content: `1. interference   2. invasion   3. Consumption   4. procession
5. specific       6. deception   7. scarce      8. thrillers

    `

    },

    'key-8-2-11': {

        type: 'cloze',

        content: `1.B 2. G  3. L   4. K  5. F
6. D 7. H  8. N  9. E  10.I`

    },

    'key-8-2-12': {

        type: 'blank',

        content: `1. persisted in   2. have relevance to   3. becoming aware of   4. isolated from
5. in the name of  6. make a contribution  7. been occupied with  8. tip the scales `
    },

    'key-8-6-4': {

        type: 'mc',

        content: `1 D   2 A  3 C   4 C   5 D    6  B     7   D   8  C `

    },

    'key-8-6-6': {

        type: 'blank',

        content: `1 emitted  2  shoved   3 sneak   4 symbol   5 wakened   
6  lagged   7 ditch   8 countdown   9  discharged  10 rotated
    `

    },

    'key-8-6-7': {

        type: 'blank',

        content: `1 out    2 with   3 in    4  between    5  from   6 of   7 with  8  over

    `

    },

    'key-8-7-2': {

        type: 'blank',

        content: `1.severely restricted  2. terrifying effects    3.subsequent advances
4.damaging impact     5.remain undiscovered

    `

    },

    'key-8-7-3': {

        type: 'blankB',

        content: `1.	1.entire field   2.subsequent advances  3.reasonable argument   4.severely restricted  
5.terrifying effects  6.suitable alternatives  7.well-understood principles  8.Medical research  9.supreme desire  10.moral compass  11.remain undiscovered    12.innocent animals

    `

    }

}
